...
export const useCart = defineStore('cart', () => {
  ...
  const addToCart = async (product) => {
    ...
    try {
      ...
      const variationId = product.variations?.length ?  selectedVariations.value[product.id] :null
     // Then you never send variation_id for products with no variations, and backend can accept null.

      if (hasVariants && !variationId) {
        return toast.warn('Please select a variation first')
      }

      loading.value = true

      const res = await api.post('/api/V1/add/cart', {
  ...
  return { items, fetchCart, moveWishlistToCart, selectedVariations, loading, increment, decrement, cartCount, cartTotal, removeFromCart, updateQuantity, applyDiscount, discount, cartTotalAfterDiscount, addToCart }
})
